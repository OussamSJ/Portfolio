...
export default function FormationComponent({ titre, ecole, date, image, index }) {
  ...
  return (
    <motion.div
      className={`relative mb-20  border-primary-300 transform -translate-x-1/2
      ...
      viewport={{ once: true }}
    >
      <div className={`absolute top-0 left-1/2 right-1/2  border-l-2 w-6 h-6  bg-white rounded-full border-2 transform -translate-x-1/2 border-primary`}/>
